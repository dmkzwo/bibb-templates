const Lightbox = (($) => {
	...
	class Lightbox {
		...
		constructor($element, config) {
            ...
            let h4 = `<h3 class="modal-title">${this._config.title || "&nbsp;"}</h3>`;
            let btn = `<button type="button" class="close" data-dismiss="modal" aria-label="${this._config.strings.close}"><span aria-hidden="true"><svg><use href="/images/sprites/misc.svg#close" xlink:href="/images/sprites/misc.svg#close"></use></svg></span></button>`;

            let header = `<div class="modal-header${this._config.title || this._config.alwaysShowClose ? '' : ' hide'}">`+(this._isBootstrap3 ? btn : btn)+`</div>`;
            let footer = `<div class="modal-footer${this._config.footer ? '' : ' hide'}"></div>`;
		...
		_resize( width, height ) {
            ...
            this._$modalDialog.css('maxWidth', width);  // IE 11 Bugfix
